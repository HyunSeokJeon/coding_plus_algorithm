# Chapter02-2
# 파이썬 심화

# Sepcial Method(Magic Method)

# 매직메소드 실습
# 파이썬의 중요한 핵심 프레임워크 -> 시퀀스(Sequence), 반복(Iterator), 함수(Functions), 클래스(Class)

# 매직메소드 기초 설명
# 기본형

# 이미 만들어진 연산자들의 집합

# 기본형
# 파이썬의 모든 자료형은? 데이터 자료형에서 객체로 취급한다!
print(int)

# 모든 속성 및 메소드 출력
print(dir(int))

# __??__ 이것들이 메직메소드, 스페셜메소드
print()
print()

n = 100
# 사용
print('EX1-1 - ', n + 200)
# 이 더하기를 파이썬에서 어떻게 만들었을까?
print('EX1-2 - ', n.__add__(200))
# 사실은 + 하던게 __add__ 로 치환이 되어 우리가 300의 값을 얻는다는 것!
print('EX1-3 - ', n.__doc__)
# 원리까지는 아니더라도 매직메소드 뭐가 뭔지는 알아야지
print('EX1-4 - ', n.__bool__(), bool(n))
print('EX1-5 - ', n * 100, n.__mul__(100))

print()
print()

class Student:
    def __init__(self, name, height):
        self._name = name
        self._height = height
    
    def __str__(self):
        return 'Student Class Info : {}, {}'.format(self._name, self._height)

    def __ge__(self, x):
        print('Called. >> __ge__ Method.')
        if self._height >= x._height:
            return True
        else:
            return False

    def __le__(self, x):
        print('Called. >> __le__ Method.')
        if self._height <= x._height:
            return True
        else:
            return False

    def __sub__(self, x):
        print('Called. >> __sub__ Method.')
        return abs(self._height - x._height)
# 인스턴스 생성
s1 = Student('James', 181)
s2 = Student('Mie', 165)

# 이번 강의의 핵심
# print(s1 + s2)  # 객체를 더하는 이것이 가능한가 

# 매직메소드 출력
print('Ex2-1 - ', s1 >= s2)
print('Ex2-2 - ', s1 <= s2)

print('Ex2-3 - ', s1 - s2)
print('Ex2-4 - ', s2 - s1)

# 객체를 객체사이의 관계로 만들 수 있다.

print('Ex2-5 - ', s1)
print('Ex2-6 - ', s2)

print()
print()

# 클래스 예제

# 백터(Vector) # Numpy에서 주로 사용한단다.

class Vector(object):
    def __init__(self, *args):
        """
        Create a vector, example : v = v = Vector(1, 2)
        """
        if len(args) == 0:
            self._x, self._y = 0, 0
        else:
            self._x, self._y = args

    def __repr__(self):
        """Returns the vector informations"""
        return 'Vector(%r, %r)' % (self._x, self._y)
    
    def __add__(self, another):
        """Returns the vector addition of self and anther"""
        return Vector(self._x + another._x, self._y + another._y)

    def __mul__(self, y):
        return Vector(self._x * y, self._y * y)

    def __bool__(self):
        return bool(max(self._x, self._y))

# Vector 인스턴스 생성
v1 = Vector(3, 5)
v2 = Vector(15, 20)
v3 = Vector()

# 매직메소드 출력
print('EX3-1 - ', Vector.__init__.__doc__)
print('EX3-2 - ', Vector.__repr__.__doc__)
print('EX3-3 - ', Vector.__add__.__doc__)
print('EX3-4 - ', v1, v2, v3)
print('EX3-5 - ', v1 + v2)
print('EX3-6 - ', v1 * 3)
print('EX3-7 - ', v2 * 10)
print('EX3-8 - ', bool(v1), bool(v2))
print('EX3-9 - ', bool(v3))

print()
print()
print()






